...
<template>
  <div>
    <div class="mb-6">
      ...
      <p class="text-body-2 text-grey-darken-1 mt-1">Manage your security account</p>
    </div>
      ...
      <v-col cols="12" md="4">
        <v-card rounded="lg" elevation="1" class="pa-6 text-center mb-4">
          <v-avatar size="100" color="secondary" class="mb-4">
            ...
            <span v-else class="text-white text-h4 font-weight-bold">{{ user?.first_name?.charAt(0)?.toUpperCase() }}</span>
          </v-avatar>
          <p class="text-h6 font-weight-bold">{{ user?.first_name }} {{ user?.last_name }}</p>
          <v-chip size="small" color="warning" class="mt-1">Security</v-chip>
          <p class="text-body-2 text-grey mt-2">{{ user?.email }}</p>
          <v-btn color="secondary" variant="tonal" class="mt-4" prepend-icon="mdi-camera" @click="$refs.fileInput.click()">
            Change Photo
          </v-btn>
